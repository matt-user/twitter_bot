...
def set_rules(url, headers):
    ...
    rules = [
        {"value": "giveaway -is:reply -is:retweet lang:en", "tag": "giveaway"},
    ]
...
def handle_tweet(json_response, oauth):
    ...
    if any(word in tweet_text for word in RETWEET_WORDS):
        handle_request(oauth.post, f"https://api.twitter.com/1.1/statuses/retweet/:{tweet_id}.json", 200, "Cannot retweet tweet")

    # TODO we can only like 1000 tweets per 24 hours
    if any(word in tweet_text for word in LIKE_WORDS):
        handle_request(oauth.post, f"https://api.twitter.com/1.1/statuses/favorites/:create.json", 200, "Cannot like tweet", params={"id": tweet_id})
    
    # scrape links, if appropriate
...
def follow_users(oauth, json_response):
    ...
    # follow the user
    handle_request(oauth.post, "https://api.twitter.com/1.1/friendships/create.json", 200, "Cannot follow user", params={"user_id": user_id})
    # Follow all of the mentioned users
    try:
        for user in json_response["data"]["entities"]["mentions"]:
            handle_request(oauth.post, "https://api.twitter.com/1.1/friendships/create.json", 200, "Cannot follow user", params={"screen_name": user["username"]})
    except KeyError:
        pass
